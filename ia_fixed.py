...
INIT_CONFIG = dict(embed_dim=96, n_heads=4, n_layers=3, dropout=0.1, use_moe=True, n_experts=2, top_k=1)
LEARN_SUP_STEPS = 400
LEARN_RL_TRIES = 300
                ...
                try:
                    if nome_arquivo.lower().endswith('.csv'):
                        ...
                        if trecho:
                            ...
                            print(f'Lido (CSV, {categoria}): {nome_arquivo} ({len(trecho)} chars extraídos)')
                        else:
                            print(f'CSV vazio/ignorado: {nome_arquivo}')
                    else:
                        with open(caminho_completo, 'r', encoding='utf-8', errors='ignore') as f:
                            resultado[categoria] += f.read() + '\n'
                            print(f'Lido ({categoria}): {nome_arquivo}')
                except Exception as e:
...
def _get_batch_de_texto(texto_encoded: torch.Tensor, bs=4):
...
def _build_flask_app(model, rl_opt, sup_opt, pretrain_opt, state):
    app = Flask(__name__)
    ...
    def status():
        with _server_lock:
            return jsonify({'gen': state.get('gen', 0), 'n_params': state.get('n_params'), 'vocab_size': VOCAB, 'config': model._cfg})
    return app


def run_server(wiki: bool=False, host: str='0.0.0.0', port: int=5000):
    if Flask is None:
    ...
    if wiki:
        ...
        t.start()
    app = _build_flask_app(model, rl_opt, sup_opt, pretrain_opt, state)
    console.print(f'[bold green]🚀 Servidor em http://{host}:{port}  (GET /v1/model, POST /v1/chat, GET /v1/status)[/bold green]\n')
    ...
    if '--serve' in sys.argv:
        ...
        if '--port' in sys.argv:
            try:
                ...
                pass
        run_server(wiki='--wiki' in sys.argv, port=_port)
    else:
